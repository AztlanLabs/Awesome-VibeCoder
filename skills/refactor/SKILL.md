...
Use this skill to clean up, modularize, and improve codebase design and maintainability without changing observable runtime behavior.

## Trigger Conditions

Invoke this skill when:
- The user requests to "clean up", "refactor", or "improve" specific code files.
- You identify code smells (long methods, duplicate logic, primitive obsession, nested conditionals).
- Code complexity must be reduced to make feature additions or testing feasible.

## Centralized State Architecture

On startup, verify the `.sdlc/` workspace state directory. Load the shared state baseline and record all refactoring logs, patterns, and progress inside `.sdlc/` state files.

1. Read `.sdlc/systemPatterns.md` on startup to align refactoring edits with established team conventions, design patterns, and naming guidelines.
2. Log refactoring changes and metrics (such as complexity reduction or files cleaned) to `.sdlc/progress.md` using the append-only pattern.

## Core Capabilities

- **Code Smell Remediation**: Refactor long functions, split God classes, extract variables/types, and remove duplication.
- **Type Safety Introduction**: Replace untyped objects and magic constants with strong typescript types, enums, or domain models.
- **Design Pattern Application**: Apply behavioral and structural patterns (Strategy, Chain of Responsibility, Builder, Guard Clauses) to simplify code flow.

## Inputs & Outputs

### Inputs
- Source files or blocks of code in the active workspace.

### Outputs
- Refactored source files displaying improved readability, modularity, and testability.

---

## Boundaries & Guardrails

- **Zero Functional Drift**: Refactoring MUST preserve original functionality. Never introduce new features, fix unrelated bugs, or change API responses under this skill.
- **Verification First**: Always run tests before starting and after every incremental change. Do not commit or deliver code changes if tests fail.
- **Preserve Git Cleanliness**: Keep edits minimal and focused. Do not refactor unrelated files outside the target scope.

---

## Verification Checklist

Before completing any refactoring work, verify that:
- [ ] Refactored code compiles and runs without diagnostics or warnings.
- [ ] **Tests have been run and verified to show zero failures**.
- [ ] No behavioral or contract changes have been introduced.
- [ ] Extracted components obey the Single Responsibility Principle.
- [ ] All magic values and dead code have been resolved.
